"""Servidor MCP do Power BI — ponto de entrada principal.

Disponibiliza ferramentas, recursos e prompts do Power BI para clientes MCP (Cline, Continue, Claude).
Inclui todos os mecanismos de controle e segurança: RBAC, limite de taxa (rate limit), auditoria, DLP e fluxo de aprovação.
"""
...
# Configurando o registro de logs estruturados 
structlog.configure(
    processors=[
...
# Estado Global ( iniciando ciclo de vida ) 
class AppState:
    settings: Settings
    auth: AzureADAuth
...
# Vida Util(inicializando/finalizando) 
@asynccontextmanager
...
# Helper: Extrair contexto do usuário do cabeçalho da solicitação: 
